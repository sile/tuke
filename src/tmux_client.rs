use std::io::{BufRead, BufReader, Write};
use std::process::{Command, Stdio};

use crate::error::{Error, Result};

/// Control mode client for tmux communication
///
/// Doc: <https://github.com/tmux/tmux/wiki/Control-Mode>
#[derive(Debug)]
pub struct TmuxClient {
    stdin: std::process::ChildStdin,
    reader: BufReader<std::process::ChildStdout>,
}

impl TmuxClient {
    pub fn new() -> Result<Self> {
        // Start tmux in control mode (-C) attached to the default session
        let mut child = Command::new("tmux")
            .arg("-C")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()?;

        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| Error::message("failed to open stdin of the tmux control client"))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Error::message("failed to open stdout of the tmux control client"))?;
        let reader = BufReader::new(stdout);

        Ok(Self { stdin, reader })
    }

    pub fn send_command(&mut self, command: &str, args: &[&str]) -> Result<()> {
        let mut cmd_string = command.to_string();
        for arg in args {
            cmd_string.push(' ');
            if *arg == "'" {
                cmd_string.push_str("\"'\"");
            } else {
                cmd_string.push('\'');
                cmd_string.push_str(arg);
                cmd_string.push('\'');
            }
        }

        // Send command to control mode client
        writeln!(self.stdin, "{}", cmd_string)?;
        self.stdin.flush()?;

        // Read response until %end or %error marker
        let mut response = String::new();
        loop {
            response.clear();
            self.reader.read_line(&mut response)?;

            // Control mode responses are wrapped in %begin/%end or %begin/%error
            if response.starts_with("%end") {
                return Ok(());
            }
            if response.starts_with("%error") {
                return Err(Error::Tmux(format!("tmux command failed: {cmd_string}")));
            }
        }
    }
}
